...
def converter_e_formatar(valor, formato):
    """Limpa resíduos e formata sem duplicar o símbolo de %."""
    ...
    if formato == 'moeda':
        ...
        # Se for decimal (ex: 0.66), vira 66%. Se for 66.0, continua 66%.
...
def load_data(file_path):
    ...
    for enc in encodings:
        try:
            ...
            df.columns = [c.strip() for c in df.columns] # Limpa espaços nos nomes
            return df
        except: continue
    return None
...
if not os.path.exists("Painel Entes.csv"):
    ...
    # Criar DF numérico para cálculos
    df_calc = df.copy()
    ...
    if df_final.empty:
        ...
        with tab3:
            tipo_view = st.radio("Mostrar rateio por:", ["Porcentual (%)", "Valor (R$)"], horizontal=True, key="rateio_view")
